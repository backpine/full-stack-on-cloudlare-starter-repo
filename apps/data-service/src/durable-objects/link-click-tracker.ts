...
export class LinkClickTracker extends DurableObject {
    ...
    async addClick(latitude: number, longitude: number, country: string, time: number) {
		...
		if (!alarm) {
            const twoSeconds = moment().add(2, "seconds").valueOf();
            await this.ctx.storage.setAlarm(twoSeconds);
        }
        }
        catch (error){
            console.error("error getting/setting the alarm", error)
        }
    }
    ...
    async alarm() {
        try{
            ...
            console.error("issue within the alarm",error)
        }
    ...
    webSocketClose(ws: WebSocket, code: number, reason: string, wasClean: boolean): void | Promise<void> {
        console.log("client closed reason and wasClean??", reason, wasClean); 
    }
}
